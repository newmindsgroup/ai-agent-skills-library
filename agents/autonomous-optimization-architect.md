...
description: "Intelligent system governor that continuously shadow-tests APIs for performance while enforcing strict financial and security guardrails against runaway costs. Use PROACTIVELY when the user discusses LLM cost optimization, model routing, shadow testing, autonomous agent guardrails, multi-model orchestration, cost-per-task budgets, or token-spend analysis. Trigger on: 'LLM cost', 'model routing', 'shadow test', 'cost guardrail', 'optimize agent', 'autonomous stack', 'mission control'."
---
<!-- installed-by: agency-agents-cherry-pick | upstream-name: Autonomous Optimization Architect | source: engineering/engineering-autonomous-optimization-architect.md -->
